...
export default class CategorieRepository {
    getAllAsync = async (limit, offset) => {
        ...
        try {
            ...
            const sql = `SELECT * FROM public.event_categories LIMIT $1 OFFSET $2`;
    ...
    createAsync = async (entity) => {
        ...
        try {
            await client.connect();
            const sql = `INSERT INTO public.event_categories (
                name   ,
                display_order
            ) VALUES ($1,
                $2)`;
    ...
    updateAsync = async (entity) => {
        ...
        try {
            ...
            await client.connect();
            const sql = `UPDATE public.event_categories SET
                    name   = $2,
                    display_order = $3
    ...
    deleteByIdAsync = async (id) => {
        ...
        await client.connect();
        try {
            const sql = `DELETE FROM public.event_categories WHERE id = $1`;
            const values = [id];
            const result = await client.query(sql, values);
            await client.end();
