...
export default class UbicacionesEventos{
    ...
    getByIdANDUserCreatorAsync = async (id, id_user) => {
        ...
        await client.connect();
        try {
            const sql = `SELECT * FROM Public.event_locations WHERE id_creator_user = $2 AND id = $1 `;
    ...
    createByUserCreatorAsync = async (entity) => {
        ...
        await client.connect();
        try {
            const sql = `INSERT INTO public.event_locations(
    ...
    async findById(id) {
        ...
        await client.connect();
        try {
            const sql = `SELECT * FROM public.locations WHERE id = $1`;
            const values = [id];
            const result = await client.query(sql, values);
            await client.end();
    ...
    upDateLocationAsync = async (entity) =>{
        ...
        try {
            ...
            return 0;}
            await client.connect();
            const sql = `UPDATE public.event_locations SET
                    id_location   = $2,
                    name = $3,
                    full_address = $4,
                    max_capacity = $5,
